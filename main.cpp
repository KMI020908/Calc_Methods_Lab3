...
const std::string &L_U_ERROR_FILE_PATH, const std::string &L_CH_ERROR_FILE_PATH, std::size_t numOfErrors = 100){
    ...
    getInterpolationErrorsLagrangeUniform(f, firstX, lastX, numOfErrors, uniErr);
    writeVectorFile(uniErr, L_U_ERROR_FILE_PATH);

    std::vector<Type> chebErr;
    getInterpolationErrorsLagrangeChebyshev(f, firstX, lastX, numOfErrors, chebErr);
...
const std::string &S_CH_ERROR_FILE_PATH, std::size_t numOfErrors = 100){
    std::vector<Type> xGrid, fGrid;
